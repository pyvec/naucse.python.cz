...
\newcommand\plpage{
    ...
    \node[opacity=0.1,below left,xshift=1cm,yshift=1cm] at (current page.north east) {\includegraphics[width=12cm]{pylady-pink}};
    \end{tikzpicture}
}
...
\\Zadání taky, ale pro úplnost: je na \url{python.cz/pyladies/s001-install}

Seznam se se sousedkami a začni s instalací; jakékoli otázky rádi zodpoví pomocníci.
V 18:00, až budeme všichni, povím pár slov na úvod a naučíme se základy práce s příkazovou řádkou. Pak instalaci dokončíme.
...
Můj adresář se soubory k PyLadies je:
...
0. Otevření příkazové řádky: &\\
1. Puštění virtuálního prostředí: & \\
...
\item Závorky v Pythonu fungují jako v matematice.
    ...
    \answerspace
\item Co dělá s čísly operátor \verb+%+ (procento)?
    \answerspace
...
\item Python umí i jiné věci než čísla. Třeba takové \emph{řetězce} – slova, věty, nebo jiný text.
    ...
    'Ahoj!'
    \end{verbatim}
\item Řetězce jsou spojovat sčítáním:
    \begin{verbatim}
    >>> 'A' + "B"
    'AB'
    \end{verbatim}
