...
Některé úlohy jsou pekelně těžké. Nepředpokládám, že je všechny vyřešíš, ale u každé se o to pokus.
...
\item Jak se jmenuje druh chyby, která nastane, když...
    \begin{itemize}
        \item Zapomeneš dát uvozovky kolem řetězce? — \texttt{SyntaxError}
        \item Zkusíš sečíst číslo a řetězec? —
        \item Dělíš nulou? —
        \item Použiješ proměnnou, do které není nic přiřazené? —
        ...
        \item Neuzavřeš uvozovky u řetězce? –
...
\item \emph{\small Podívej se na odpovědi na předchozí otázku, ale Python zkus použít jen na ověření.}
    \\Jaká chyba nastane, když zkusíš p číslo od řetězce?
...
\item Lomítko (\texttt{/}) je operátor, ale nedá se použít na řetězce. Vykřičník (\texttt{!}) v Pythonu není operátor.
    ...
    \\Je zavináč (\texttt{@}) v Pythonu operátor?  % odpověď na tohle se v Pythonu 3.5 změní. To bude sranda!
...
\item Které Pythoní operátory dokážeš z hlavy vyjmenovat?
...
\item Zkusila jsi porovnávat řetězce?
...

    {
        ...
        \verb+1+ & \verb+1+ & \True & \False & \True \rowend
        \verb+1+ & \verb+2+ & \False & \False & \True \rowend
        \verb+'abc'+ & \verb+'abc'+ & \True & \False & \True \rowend
...
\item Jaká je hodnota proměnné po provedení příkazu: \hspace{1cm}\verb+promenna = 1 < 2+ \hspace{0.2cm}?
    \\Co když do proměnné přiřadíš \verb+2 < 2+ ?
    \\\verb+1 < 2 < 3+ ?
    \\\verb+1 < 3 < 2+ ?
    \\\verb+1 < 3 < 3+ ?
    \\\verb+'abc' < 'ABC' < 'def' < 'zajíc'+ ?
    \\Pokud je hodnota proměnné \False, dají se některé z porovnávaných hodnot vyměnit, aby byla \True?
